export default function weatherRender() {
    ...
    async function checkWeatherIcon(iconValue) {
        ...
        const freezingCond = ["snow", "snow-showers-day", "snow-showers-night"];
        ...
        const pictureFilePath = await import(`../images/${endCondition}.png`);
    ...
    async function defaultRender(weatherData) {
        ...
        */
        ...
        for (let i = 1; i < dayCutoff; i += increment) {
            const iconObj = await checkWeatherIcon(processedData.days[i].icon);
